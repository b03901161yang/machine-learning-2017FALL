...
def trainModelLinear_1d(trainData, trainLabel):#use adagrad here, has bug now
    ...
    sum_g_w = 0
    sum_g_b = 0
    for t in range(500000):#1000 iteration
        if(t%100 == 0):
            print('t = ',t)
            w_vec[0] = b
...
def trainModelLinear_2d(trainData, trainLabel):#use adagrad here, has bug now
    ...
    sum_g_w1 = 0
    sum_g_b = 0
    for t in range(500000):#300 iteration for easy demo
        if(t%1000 == 0):
            print('t = ',t)
            w_vec[0] = b
        ...
        #print('train_label ',trainLabel.shape)
        del_y_arr = np.transpose(trainLabel) - ( np.dot( np.transpose(w1_2d) ,np.transpose(trainData))  + np.dot( np.transpose(w2_2d), np.transpose((trainData)**2)) )
        ...
        #print('g_w2_t = ',g_w2_t.shape)
        #print('g_w1_t = ',g_w1_t.shape)
        #print('g_b_t = ',g_b_t.shape)
        #for n in range(num_of_data):
        #    tmp_x = trainData[n]
        #    tmp_x2 = (tmp_x)**2
